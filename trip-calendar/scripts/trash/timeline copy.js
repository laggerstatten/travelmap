(function() {
    ...
    <label>Location
      ...
      }" proximity="" autocomplete="true"></mapbox-search-box>
    </label>
    <label>Start
  ...
  `;
            ...
            const searchBoxEl = editor.querySelector(`#searchbox-${e.id}`);
            if (searchBoxEl) {
                searchBoxEl.accessToken = mapboxgl.accessToken;
                searchBoxEl.addEventListener('retrieve', (ev) => {
                    const feature = ev.detail ? .features ? .[0];
                    if (feature && feature.geometry) {
                        e.lat = feature.geometry.coordinates[1];
                        e.lon = feature.geometry.coordinates[0];
                        e.location_name =
