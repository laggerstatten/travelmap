...
async function insertStopInNearestRoute(stop, segments) {
  ...
  const drives = segments.filter((ev) => ev.type === 'drive' && ev.routeGeometry);
  if (!drives.length) {
    return;
  }
  ...
  const destination = segments.find((seg) => seg.id === bestDrive.destinationId);
