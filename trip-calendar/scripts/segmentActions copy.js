...
function updateDriveDurations(editor) {
  const segId = editor.closest('.segment')?.dataset.id;
  const seg = loadSegments().find(s => s.id === segId);
  if (!seg) return;
  ...
  const baseHr = parseFloat(seg.durationHr) || 0;
