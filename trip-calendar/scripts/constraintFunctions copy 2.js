        ...
        <select class="param-operator" data-cid="${cid}" data-param="${paramName}">
            ${opts
              .map(
                ...
                ${value === o.value || (!value && i === 0) ? 'selected' : ''}>
                ${o.label}
            </option>`
              )
              .join('')}
  ...
  function wireEvents() {
    ...
    listEl.querySelectorAll('.mode-select').forEach((sel) => {
      sel.onchange = (e) => {
        ...
        const t = detectParamType(p, c.type);

        if (t === "dateRangeSingle") {
            c.params[p] = { startDate: null, endDate: null };
        } else if (t === "multiDate") {
            c.params[p] = [];
        } else if (t === "daysOfWeek") {
            c.params[p] = [];
        } else if (t === "windowsList") {
            c.params[p] = [];
        } else {
            c.params[p] = "";
        }
    ...
    listEl.querySelectorAll('.param-operator').forEach((sel) => {
      ...
      // If operator missing, initialize from the <select>
      if (!c.params.operator) {
    ...
    // Ensure value exists
    if (!c.params[param] || typeof c.params[param] !== "object") {
        c.params[param] = { startDate: null, endDate: null };
    }

    const { startDate, endDate } = c.params[param];
