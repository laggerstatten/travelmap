...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let latField = 'lat';
    if (table == 'aza') {
      fields = 'aza_id,Name,City,State,CenterPointLong,CenterPointLat';
      lonField = 'CenterPointLong';
      latField = 'CenterPointLat';
    }
    ...
    const results = data
      .map((r) => {
        const recLat = r[latField];
        const recLon = r[lonField];

        if (recLat == null || recLon == null) return null;

        const dLat = ((recLat - lat) * Math.PI) / 180;
        const dLon = ((recLon - lng) * Math.PI) / 180;
        const lat1 = (lat * Math.PI) / 180;
        const lat2 = (recLat * Math.PI) / 180;
        ...
        return { ...r, distance_mi };
      })
      .filter((r) => r && r.distance_mi <= radius_miles)
