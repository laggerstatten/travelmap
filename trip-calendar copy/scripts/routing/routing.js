...
async function removeStopAndHealRoute(stop, list, idx) {
  ...
  const leftDrive  = list[idx - 1];
  const rightDrive = list[idx];
  const destination = list[idx + 1];

  if (!leftDrive || !rightDrive || !destination) return list;
  if (leftDrive.type !== "drive" || rightDrive.type !== "drive") return list;

  const origin = list.find(s => s.id === leftDrive.originId);
  ...
  // Now remove ONLY [leftDrive, rightDrive]
  list.splice(idx, 1);     // remove rightDrive (current idx)
  list.splice(idx - 1, 1); // remove leftDrive (now at idx - 1)

  // Insert merged drive at the position of the old leftDrive
