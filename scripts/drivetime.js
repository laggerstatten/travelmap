async function fetchMajorUA(lat, lng) {
    ...
    try {
        ...
        console.error('Error retrieving major UAs or drive times:', err);
    }
}
...
function updateUATable(rows) {
    ...
    rows.forEach((r) => {
        ...
        tr.innerHTML = `
                  ...
                  <td>${r.drive_time_min ? r.drive_time_min.toFixed(1) : ''
            }</td>
                  <td>${r.drive_distance_mi ? r.drive_distance_mi.toFixed(1) : ''
            }</td>
                `;
        ...
        tr.addEventListener('click', async function() {
            ...
            // remove old route and destination marker
...
function updateMajorUATable(rows) {
    ...
    `;
        ...
        tr.addEventListener('click', async function() {
            ...
            const destMarker = new mapboxgl.Marker({
                color: '#006400'
            }); // dark green to distinguish major UA
            destMarker.setLngLat([destLon, destLat]);
            destMarker.addTo(map);
            window.destMarker = destMarker;
            ...
            const popupText = `<b>${r.text_ua || 'Urban Area'}</b><br>
        Pop: ${r.val_pop_ua?.toLocaleString?.() || 'N/A'}<br>
        Drive: ${r.drive_time_min ? r.drive_time_min.toFixed(1) : '?'} min (${r.drive_distance_mi ? r.drive_distance_mi.toFixed(1) : '?'
                } mi)`;
            destPopup.setLngLat([destLon, destLat]);
            destPopup.setHTML(popupText);
            destPopup.addTo(map);
            ...
            try {
                ...
                map.addLayer({
                    ...
                    paint: {
                        ...
                        'line-width': 4,
                        'line-opacity': 0.8
                    } // greenish color for distinction
                });
